...
Generate group-level ROI time-series and ROI-mask visualizations from
per-configuration image-space results created by the FIG8 pipeline. This
module selects ROIs using t-stat or magnitude images, computes weighted
group averages (accounting for within- and between-subject variance), and
plots HbO/HbR traces with shaded standard-error regions.

Usage
-----
Edit the CONFIG section (ROOT_DIR, CMEAS_FLAG, FNAME_FLAG, ROI_SELECTION,
cfg_list, etc.) and run::
...
Outputs
-------
- PNG time-series plots and optional ROI mask images under
    <ROOT_DIR>/derivatives/cedalion/plots/image_space/.

Dependencies
------------
- numpy, xarray, pyvista, matplotlib and project helper modules loaded
    from the modules/ directory (image_recon_func, get_image_metrics).
...
for cc, cfg in enumerate(cfg_list):
    ...
    for trial_type in mag.trial_type:
        ...
        X_between_subj = (
            X_between_subj.mean("subj") * X_mean_within_subj
        ...
        max_tstat = X_ts_roi_brain.sel(chromo="HbO", time=slice(5, 8)).mean("time") / X_var_roi_brain.sel(
            chromo="HbO", time=slice(5, 8)
        ).mean("time")
        ...
        ax[0].text(
            0.97,
            0.95,
            rf"$t_{{\mathrm{{peak}}}} = {np.round(max_tstat.values, 2)}$",
...
if PLOT_MASK:
    ...
    if SAVE:
        ...
        filepath = os.path.join(
            SAVE_DIR, f"task-{TASK}_image_mask_{Cmeas_name}_right_roi-{ROI_SELECTION}_{NOISE_MODEL}.png"
        )
        p0_right.screenshot(filepath)

        filepath = os.path.join(
            SAVE_DIR, f"task-{TASK}_image_mask_{Cmeas_name}_left_roi-{ROI_SELECTION}_{NOISE_MODEL}.png"
        )
