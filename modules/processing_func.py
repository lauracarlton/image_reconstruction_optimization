...
def preprocess(rec, cfg_preprocess):
    ...
    rec['od_p'] = motion.tddr(rec['od_p'])
    rec['od_o'] = motion.tddr(rec['od_o'])
    
    rec['od_o'] = rec['od_o'].where( ~rec['od_o'].isnull(), 1e-18 ) 

    
    rec["od_o"] = cedalion.sigproc.frequency.freq_filter(rec["od_o"], 
                                                                   cfg_preprocess['cfg_bandpass']['fmin'], 
                                                                   cfg_preprocess['cfg_bandpass']['fmax'])
    
    rec["od_p"] = cedalion.sigproc.frequency.freq_filter(rec["od_p"], 
                                                                   cfg_preprocess['cfg_bandpass']['fmin'], 
                                                                   cfg_preprocess['cfg_bandpass']['fmax'])

    
    rec['conc_o'] = cedalion.nirs.od2conc(rec['od_o'], rec.geo3d, dpf)
    rec['conc_p'] = cedalion.nirs.od2conc(rec['od_p'], rec.geo3d, dpf)
    
    rec = GLM(rec, 'conc_o', rec.stim, cfg_preprocess['cfg_GLM'])    
    rec = GLM(rec, 'conc_p', rec.stim, cfg_preprocess['cfg_GLM'])
    
    rec['od_o_postglm'] = cedalion.nirs.conc2od(rec['conc_o_postglm'], rec.geo3d, dpf)
    rec['od_p_postglm'] = cedalion.nirs.conc2od(rec['conc_p_postglm'], rec.geo3d, dpf)
...
def preprocess_batch(cfg_dataset, cfg_preprocess):
    ...
    for subj_idx in range(n_subjects):
        ...
        for file_idx in range(n_files_per_subject):
            ...
            recTmp.stim = stim_df
            recTmp['amp'] = recTmp['amp'].sel(channel=cfg_preprocess['cfg_prune']['channel_sel'])
